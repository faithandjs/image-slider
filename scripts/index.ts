...
const getInactive = (
  num: number,
  inactives: NodeListOf<HTMLDivElement>,
  curr: number
) => {
  const zero = Array.from(boxes).findIndex(
    (el) => Array.from(inactives)[0] === el
  );
  const one = Array.from(boxes).findIndex(
    (el) => Array.from(inactives)[1] === el
  );
  ...
  if (curr === asc[1]) {
    ...
    return Array.from(boxes).findIndex(
      (el) => Array.from(inactives)[num] === el
    );
  }
};

const getPrevOrNext = (count: number, type: 'p' | 'n') =>
  type === 'p'
    ? count === 0
      ? totalIds
      : count - 1
